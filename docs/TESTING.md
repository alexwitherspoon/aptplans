...
- Builder writes airport, state, and document pages, RSS, a `/feeds/` index of the feed tree, a sitemap from those published URLs, `status.json`, and bulk dumps. Pass `--lid 4S9` for a partial rebuild (airport page, its state page, related documents and feeds, sitemap merge) without regenerating the full tree; `--about` refreshes only `/about/`. After `fetch`/`vet`/`check`, the worker queues a scoped `site_build` when an airport LID is known; boot and monthly refresh still queue a full build.
