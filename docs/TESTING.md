...
- NASR public-use airports merge with NPIAS role flags from fixtures; intake can admit a LID absent from NPIAS
...
- **Gated rubric evaluations** — shared pattern for plan verify, finance verify, search triage, and future tasks (e.g. grant spend category). Documented in [GATED_EVALUATION.md](GATED_EVALUATION.md) with an audit of rule-based vs LLM paths. Implementation phases in [LLM_EVALUATION_ROADMAP.md](LLM_EVALUATION_ROADMAP.md).
...
- Builder writes airport, state, and document pages, RSS, a `/feeds/` index of the feed tree, a sitemap from those published URLs, `status.json`, and bulk dumps. Pass `--full` for a full tree, or combine partial flags: `--lid`, `--state`, `--document`, `--about`, `--index`, `--airports-index`, `--states-index`, `--search`, `--feeds`, `--data`, `--global-feeds`. `python3 -m pipeline.site_build --enqueue` queues the same scope on the worker. After `fetch`/`vet`/`check`, the worker queues a scoped `site_build` (fetch: airport + data; vet/check: airport + list pages + data; `link_check`: index + airports index + data + global feeds; `pipeline_snapshot` completion: about only). Boot and monthly refresh queue a full build. Pending `site_build` jobs merge wider scopes; active jobs are left alone and a follow-up job is enqueued if needed.
- Airport funding is grouped federal / state / local / other; state pages show federal grant allocations by fiscal year and airport (maintenance vs growth via hybrid `effective_spend_category` — see [GATED_EVALUATION.md](GATED_EVALUATION.md))
- Rubric classification infrastructure: `pipeline/classify.py`, `pipeline/evaluations.py`, grant spend LLM at refresh, finance fields on vet, plan outlook and hub-link LLM when `APTPLANS_LLM=1` (mocked in CI)
- Compose stack is `site`, `search`, `worker`, and `ollama`. The worker drains the queue continuously (one airport at a time by default; `APTPLANS_AIRPORT_CONCURRENCY` and `APTPLANS_JOB_PAUSE_SEC` tune tempo). GitHub intake is at most hourly while idle. Uncaught jobs retry three times, then `needs_human`. Meilisearch and Ollama are off the host network on origin. CI does not start Meilisearch and leaves `MEILI_URL` unset. Local Compose binds `127.0.0.1:11434` for diagnostics.
