...
If HTML looks stale at the edge after 24 hours, origin Cache-Control already caps shared caches at a day. Purge Cloudflare for HTML/RSS only when a same-day fix must land immediately. `/files/` responses are `no-store` so a review revocation takes effect at the origin without a stale shared-cache copy.
