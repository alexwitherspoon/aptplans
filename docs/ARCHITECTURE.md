...
Jobs are a single on-disk queue. Compose runs four services: `site` (Caddy), `search` (Meilisearch), `worker`, and `ollama`. The worker drains `pending/` only: by default one airport at a time (`APTPLANS_AIRPORT_CONCURRENCY=1`), finishing that LID's explore/fetch/vet jobs before the next; `APTPLANS_JOB_PAUSE_SEC` (default 2) paces Brave and Ollama between jobs. Sleep when the queue is empty (`APTPLANS_WORKER_IDLE_SEC`). Periodic maintenance is enqueued by **systemd timers** (discovery, link check, snapshot, overview refresh, search sync, weekly site build, hourly GitHub intake, monthly FAA overlay). **Boot** warms Ollama and refreshes stale FAA overlays when the worker container starts. On origin, FAA fetch runs only if overlay files are missing, empty, or from a prior month. Document jobs do not refresh those overlays. Local Compose does not live-fetch FAA. Daily timers probe official URLs (HEAD, then a tiny GET if HEAD is unsupported): live, moved, or dead. Dead URLs with a preserved copy become `preserved_only`; without a copy they become `missing` and the worker tries mirrors, then Wayback CDX (`APTPLANS_WAYBACK=1` on origin) to queue a fetch. 5xx is not dead. When `overlay_refresh` completes, the worker chains grant classify, overview refresh, search sync, site build, and discovery. Claim and complete take the flock; job bodies run outside it so multiple airport slots can overlap when concurrency is above 1. Scoped HTML rebuild runs after airport jobs complete. Uncaught job errors retry from `active/` with backoff and give up after three attempts (`needs_human`).
...
The worker runs on the origin. After a fetch it writes completeness and hashes to `/var/lib/aptplans/catalog` (overlay JSONL), stores private source bytes at `/var/lib/aptplans/files/{sha256}.pdf`, and writes gated native page text at `/var/lib/aptplans/text/{sha256}.jsonl`. Only artifacts allowed by `visible_on_site()` are projected into `/var/lib/aptplans/public-files`, which is mounted at Caddy `/files/`; demotion removes the public projection without deleting the private source. It does not commit catalog JSON or PDFs to GitHub. After each successful preserve it synchronizes Meilisearch and rebuilds HTML from git plus overlay into `/var/lib/aptplans/site`.
