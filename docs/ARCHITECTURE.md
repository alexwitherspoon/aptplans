...
systemd unit, pipeline      SQLite domain generations    of Caddy
reference fixtures only     versioned static releases
...
Periodic maintenance is enqueued by **systemd timers** (discovery, link check, snapshot, overview refresh, search sync, weekly site build, hourly GitHub intake, monthly FAA overlay). Open maintenance work is deduplicated transactionally. `APTPLANS_JOB_PAUSE_SEC` (default 2) paces Brave and Ollama; `APTPLANS_WORKER_IDLE_SEC` controls idle sleep. Uncaught errors receive a durable `next_attempt_at` with bounded exponential backoff. Ordinary jobs dead-letter after three attempts; publication synchronization uses continuous retries so an incomplete release cannot be treated as complete.

## Publish path

This GitHub repository is code and test fixtures only. Origin imports the former JSONL overlay once, then the worker stores airports, grants, budgets, overviews, documents, and changes as immutable payload versions with an atomic `current_generation_id`. Every catalog reader resolves one generation and cannot mix records from concurrent writes. JSONL is only a generation-bound export/interchange format after cutover.

After a fetch the worker commits a new domain generation, stores private source bytes at `/var/lib/aptplans/files/{sha256}.pdf`, and writes gated native page text at `/var/lib/aptplans/text/{sha256}.jsonl`. Publication builds the site, visible file projection, and a generation-tagged Meilisearch index in staging. The coordinator validates required outputs and public-file visibility, removes revocations from live search, atomically changes `/var/lib/aptplans/releases/current`, swaps search indexes, and records activation. Failed projection retries the desired generation; it does not roll back authoritative review state. Caddy resolves `current/site` and `current/public-files` per request.
...
**In this GitHub repo:** builder, Compose files, systemd units, pipeline, 50 state hubs, reference-case official URLs, hashed reference PDFs under `catalog/references/files/`. Mutable airport identity, completeness, and unofficial notes live in origin domain generations.
...
**Rubric evaluations** (plan verify, finance verify, search triage, grant spend, and future tasks) share one pattern: rubric in the prompt, schema JSON out, code whitelists labels, optional rule fallback, results stored on overlay rows. See [GATED_EVALUATION.md](GATED_EVALUATION.md) for the full pattern and an audit of what is implemented vs still rule-based.
...
Every worker job appends an immutable outcome audit row in one of four buckets: **accepted** (`curated` / `auto_pass` / `published`), **uncertain** (`pending` snapshots and near-threshold evidence scores), **needs_human**, and **failed** (`dead`, `ssi`, `not_plan`). Human labels use the separate control audit ledger. Artifacts that fail a check are stored privately for **90 days** under `/var/lib/aptplans/reject` (not Caddy `/files/`, not Meilisearch).

A private review API (`python3 -m pipeline.review_api`) is HTTPS at `https://aptplans.org/review`. Caddy proxies `/review/` on `:443` only to a container with no published host port. The API lists scoring buckets and rejects, returns full preserved bytes only on explicit bytes endpoints, and accepts human labels. `PATCH /v1/documents/{id}` validates and queues a content-hash-bound transition. The API mounts the jobs/domain ledger read-only and writes only the separate control/audit ledger; the serial worker commits domain state and activates validated releases. Laptop and Cursor agent copies of the key live in gitignored `.env`. `robots.txt` disallows `/review/`.
