...
Jobs, attempts, priorities, dependencies, retry times, and leases live in the WAL-mode `queue/jobs.sqlite3` ledger. Compose runs four services: `site` (Caddy), `search` (Meilisearch), `worker`, and `ollama`. The worker transactionally claims eligible work and heartbeats its lease; an expired lease is recovered after a crash. By default it handles one airport at a time (`APTPLANS_AIRPORT_CONCURRENCY=1`), finishing that LID's work before the next. Maintenance has explicit priority, old work gains priority with age, and only one job for a given airport may be leased. The authenticated review API writes operator commands to the separate `queue/control.sqlite3` audit inbox; only the worker imports those commands into the authoritative job ledger.
