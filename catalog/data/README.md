...
Refresh on the origin when an overlay is missing, empty, or not written this calendar month (`python3 -m pipeline.refresh_airports`). GitHub Actions must not live-fetch FAA or OurAirports; CI and `make site` without an overlay use the reference cases only.
