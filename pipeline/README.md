...
The worker drains the on-disk queue with airport-scoped slots (default one airport at a time: finish PDX explore/fetch/vet before starting TTD). `APTPLANS_AIRPORT_CONCURRENCY` raises how many distinct airports may have jobs in `active/` at once; `APTPLANS_JOB_PAUSE_SEC` (default 2) sleeps after each successful job to pace Brave and Ollama. Idle poll is `APTPLANS_WORKER_IDLE_SEC` (default 60). Periodic maintenance is enqueued by **systemd timers** (discovery, link check, snapshot, overview refresh, search sync, weekly site build, hourly GitHub intake). **Boot** (worker container start) only warms Ollama and refreshes stale FAA overlays. The worker never polls intake or discovery while idle. Timers and `python3 -m pipeline.boot_jobs` enqueue jobs only; the worker runs them from `pending/`. After airport jobs complete, `pipeline_snapshot` and scoped `site_build` queue jobs refresh `pipeline.json` and static HTML (`site/build.py` skips generate when inputs are unchanged).
